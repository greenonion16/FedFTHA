        ...
        for iter in range(local_eps):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                if 'sent140' in self.args.dataset:
                    ...
                    sup_x, sup_y = data.cuda(), targets.cuda()
                    targ_x, targ_y = data.cuda(), targets.cuda()
                    ...
                    sup_x, sup_y = images[:split].cuda(), labels[:split].cuda()
                    targ_x, targ_y = images[split:].cuda(), labels[split:].cuda()
...
class LocalUpdateFTHA(object):
    ...
    def train(self, net, w_glob_keys, last=False, dataset_test=None, ind=-1, idx=-1, lr=0.1):
        ...
        for iter in range(local_eps):
            ...
            if (iter < rep_eps and self.args.alg == 'fedftha') or last:
                for name, param in net.named_parameters():
                    if name in w_glob_keys:
                        param.requires_grad = True
                    else:
            ...
            # then do local epochs for the representation
            elif iter >= rep_eps and self.args.alg == 'fedftha' and not last:
                for name, param in net.named_parameters():
                    if name in w_glob_keys:
                        param.requires_grad = False
                    else:
