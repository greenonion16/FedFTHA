...
def get_model(args):
    if args.model == 'cnn' and 'cifar100' in args.dataset:
        ...
        net_glob = model = RNNSent(args, 'LSTM', 2, 25, 128, 1, 0.5, tie_weights=False).cuda()
    else:
...
def headavg(args, local_nets_dict, head_weights_keys, user_cat_index):
        ...
        for key in head_weights_keys:
            for line in cate_set:
                if 'bias' not in key:
                    # try:
                    if args.cos:
                        # print('local_nets_dict[idx][key]', local_nets_dict[idx][key].size())
    ...
    for idx in range(dict_length):
        ...
        # print('cate_avg_weight', cate_avg_weight)
    ...
    if args.cos:
        ...
        pass
    # print(glob_head, glob_head.size())
    # quit()
    return glob_head
